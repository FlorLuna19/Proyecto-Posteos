...
consultarListaUsuarios(creaBotones);
...
function consultarListaUsuarios(cbResponse) {
    ...
    request.onreadystatechange = function () {
        if(this.readyState == 4 && this.status == 200){
            ...
            console.log("consultar lista usuarios", obj);
        }
    }
...
//         if( this.readyState == 4 && this.status == 200 ){
//             objparseado = JSON.parse(request.responseText);
//         console.log("todo ok");
//         }
...
function creaBotones(objpar){

var objParseado = objpar;

for (let i = 0; i < objParseado.length; i++) {

agregarBoton(objParseado[i].name, objParseado[i].id);


    // var boton = document.createElement("button");
    // boton.addEventListener("click", function() {   verPostsUsuario( id )   }     );

    // var name = document.createTextNode(objpar[i].name);

    // boton.appendChild(name);

    // var refdivhtml = document.getElementById("divListaBotones");
    // refdivhtml.appendChild(boton);
    
    
}
console.log("dentro del ciclo For entonces tenes botones");
}